...
int sample() {
    ...
    try {
        ...
        /* Any primitive integer can be converted implicitly to a BigInteger. */
        a = b;
...
BigInteger BigFibonacci(const long long number)
{
    auto x0 = BigInteger(0);
    auto x1 = BigInteger(1);
    for (auto i = 1; i < number; ++i)
    {
        const auto tmp = x0 + x1;
        x0 = x1;
        x1 = tmp;
    }
    return x1;
